...
import { NasaApiService, EpicImage, NasaApiResponse, EpicType } from '../nasa-api/nasa-api.service';
...
export class EpicService {
  ...
  async getAvailableDates(type: EpicType = 'natural'): Promise<EpicServiceResponse> {  
  ...
  async getEpicImages(query: QueryEpicDto): Promise<EpicServiceResponse> {
    let date = query.date;
    let type: EpicType = query.natural ? 'natural' : 'enhanced';
    const cachePath = this.cacheService.getCachePath(date, type);

    // 1. Check if cache exists
    let images = await this.cacheService.getCachedData<EpicImage[]>(cachePath);
  ...
  async getEpicImageByIdentifier(image_identifier: string, date: string): Promise<string> {
    const cachePath = this.cacheService.getImageCachePath(date, image_identifier);
  ...
  private async cleanupAndRetry(image_identifier: string, date: string, cachePath: string): Promise<string> {
    try {
      ...
      // Retry download
      this.logger.log(`Retrying download for corrupted file: ${image_identifier}, date: ${date}`);
      await this.nasaApiService.saveEpicImageByIdentifier(image_identifier, date, cachePath);
      ...
      if (await this.cacheService.fileExists(cachePath)) {
        const stats = await fs.stat(cachePath);
        if (stats.size > 0 && await this.cacheService.validatePngFile(cachePath)) {
